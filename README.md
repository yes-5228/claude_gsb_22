...
| GET | `/stats/overview` | 核心指标（全量口径，不带统计区间） |
| GET | `/stats/dashboard?mode=&days=` | 看板聚合数据；`mode=current/mom/yoy`（本期/环比/同比），环比与同比均为与本期等长、由后端统一划分的区间。指标卡、趋势、分布、区域合计基于同一份区间数据集；返回 `range`（区间日期）与 `has_inspections/has_issues`（空区间标记，均分为 `null` 而非 0） |
...
- **看板区间口径**：看板支持「本期 / 环比区间 / 同比区间」三种口径与 7/14/30 天区间长度。环比是紧邻本期的上一个等长区间，同比是去年同一日历区间（2 月 29 日回退到 28 日）。区间内的巡查与问题数据在服务端只查询一次，因此指标卡合计、按天趋势、状态/分类/严重程度/区域各分布合计必然一致；看板下钻到明细列表时自动带上同一区间的 `date_from/date_to` 与分类、区域等筛选条件，明细总数可与看板直接对账。区间口径通过 URL 参数传递并缓存在 sessionStorage，明细页点「返回看板」恢复原口径。区间没有数据时看板明确提示「该区间无数据」，均分与闭环率显示「无数据」（接口为 `null`）而不是 0；公厕数、未闭环、超期等存量指标始终取当前时点，不随区间变化。口径切换时看板整块随同一次响应原子刷新，不会出现新旧数据混杂。
