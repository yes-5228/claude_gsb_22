...
| 模块 | 页面/入口 | 主要能力 |
| --- | --- | --- |
| 总览看板 | `/`（支持 `?days=&compare=mom\|yoy`） | 环比/同比同口径区间切换、核心指标卡（含对比脚注）、巡查与问题趋势、整改状态/分类/严重程度分布、区域运行情况、重点关注公厕、最新问题与巡查；点击分类/区域/数值直接进对应明细并自动带上区间与筛选，返回看板保留原区间 |
...
| GET | `/stats/dashboard` | 看板聚合数据（`trend_days` 区间天数、`compare=mom/yoy` 环比/同比；趋势、分布、区域、排行、最新记录同一区间口径） |
...
- **看板区间口径**：本期固定为“截止今天的近 N 天”（N∈{7,14,30}）；环比=紧邻的等长上一区间、同比=去年同期等长区间（2/29 自动收敛到 2/28），三者长度与边界完全一致。巡查按 `inspect_time`、问题按 `report_time` 落桶，指标卡、趋势、分类/严重程度/状态/区域分布与明细列表的合计因此严格相等；区间内确无巡查时均分/闭环率返回 `null`（前端显示“—”并明确提示无数据），而不是显示 0。区间与对比口径写在看板 URL 上，从明细返回时原区间保留；切换口径时整屏由同一次请求结果一起替换刷新。
